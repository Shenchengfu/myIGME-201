        ...
        start:
            ...
            */
            ...
            if (sDirection.ToLower().StartsWith("a"))
            {
                ...
                Array.Sort(sNumber, StringComparer.InvariantCulture);
            }
            else
            {
                //findHiLow = new sortingFunction(FindHighestValue);
                Array.Sort(sNumber, StringComparer.InvariantCulture);
                string[] reverse = new string[sNumber.Length];
                for (int i = sNumber.Length - 1; i >= 0; i--)
                {
                    reverse[sNumber.Length - i - 1] = sNumber[i];
                }
                sNumber = reverse;
            }
            ...
            */
            ...
            foreach (string thisNum in sNumber)
            {
